...
#Lee's Cipher (Keyed-substitution cipher)
...
#u = i = 14 #v = 4 = 21 #w = 9 = 22 #x = t = 16 #y = h = 24 
#z = x = 18

#def shift():
    #TO-DO:
    #make code shift with key- 30 keys, each key shifts the cypher to a specfied order
    #each user (encrypter & decrypter) must share their key number to use the program correctly
...
def cyph2(read):
    ...
    if read == "x" or read == "['x']":
        return print(*cypher1[18],end="")

def encrypt_decrypt():
    #print("Enter Key-Encryption Code. eg. 14")
    #key_in = int(input("> "))
    #key.append(key_in)
    #if key_in == int:
    #    shift()
    print("Write or Read (W or R) Coded Cyper Message?: ")
    ask_in = input("> ")
    if ask_in.upper() == "W":
        print("Your message: ")
        message = input("> ")
